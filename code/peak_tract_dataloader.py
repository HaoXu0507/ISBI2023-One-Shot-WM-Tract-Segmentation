...
class DataLoaderTraining:
    ...
    def get_batch_generator(self,subjects,subject_idx=0,type="train"):
        ...
        if type=="train":
            batch_gen = BatchGenerator2D_Nifti_random(self.batch_size, self.data_dir, self.label_dir,subjects, self.tract_name)
            batch_gen = self._augment_data(batch_gen, keys=["data", "seg", ], type=type)

        else:
            batch_gen = BatchGenerator2D_data_ordered_standalone(self.batch_size, self.data_dir, self.label_dir, subjects, self.tract_name,subject_idx)
            batch_gen = self._augment_data(batch_gen, keys=["data_x", "seg_x", "data_y", "seg_y", "data_z", "seg_z", ], type=type)
